...
async function formatAsDocx(content, title, reportType, filters) {
  const date = new Date().toLocaleDateString("en-US", {
    year: "numeric", month: "long", day: "numeric"
  });
  ...
  if (filters.departmentName) {
    children.push(
      new Paragraph({ children: [new TextRun({ text: `Department: ${filters.departmentName}` })] })
    );
  }
  if (filters.domain) {
    children.push(
      new Paragraph({ children: [new TextRun({ text: `Domain: ${filters.domain}` })] })
    );
  }
  if (filters.fiscalYear) {
    children.push(
      new Paragraph({ children: [new TextRun({ text: `Fiscal Year: ${filters.fiscalYear}` })] })
    );
  }
  ...
  for (const line of content.split("\n")) {
    ...
    if (h2Match) {
      children.push(new Paragraph({ text: h2Match[1], heading: HeadingLevel.HEADING_2 }));
    } else if (h3Match) {
      children.push(new Paragraph({ text: h3Match[1], heading: HeadingLevel.HEADING_3 }));
    } else if (bulletMatch) {
      children.push(
        new Paragraph({
  ...
  children.push(
    ...
    new Paragraph({
      children: [
        new TextRun({
          text: "DISCLAIMER: Generated by Budget AI Assistant using approved organizational documents. Verify all figures before submission.",
          italics: true,
          size: 18
        })
      ]
...
function formatAsPdf(content, title, reportType, filters) {
  return new Promise((resolve, reject) => {
    ...
    doc
      ...
      .font("Helvetica")
      .fillColor("#555555")
      .text(`Report Type: ${reportType}`, { align: "center" });

    doc.text(`Generated: ${date}`, { align: "center" });

    if (filters.departmentName) {
      doc.text(`Department: ${filters.departmentName}`, { align: "center" });
    }
    if (filters.domain) {
      doc.text(`Domain: ${filters.domain}`, { align: "center" });
    }
    if (filters.fiscalYear) {
      doc.text(`Fiscal Year: ${filters.fiscalYear}`, { align: "center" });
    }
    ...
    for (const line of content.split("\n")) {
      ...
      if (h2) {
        ...
        doc
          .fontSize(10)
          .font("Helvetica")
          .text(`  •  ${bullet[1]}`, { indent: 10, lineGap: 2 });
      } else if (line.trim() === "") {
        doc.moveDown(0.3);
      } else {
    ...
    doc
      ...
      .fillColor("#888888")
      .text(
        "DISCLAIMER: Generated by Budget AI Assistant using approved organisational documents. " +
        "Verify all figures against authoritative financial records before submission or filing.",
        { align: "center" }
      );
...
export async function generateManualReport(userId, payload) {
  ...
  try {
    ...
    // Format binary for download (docx/pdf) or plain text
    let formattedContent;
    if (format === "docx") {
      formattedContent = await formatAsDocx(rawContent, title, reportType, { domain, departmentName, fiscalYear });
    } else if (format === "pdf") {
      formattedContent = await formatAsPdf(rawContent, title, reportType, { domain, departmentName, fiscalYear });
    } else {
      formattedContent = formatAsPlainText(rawContent, title, reportType, { domain, departmentName, fiscalYear });
    }

    const wordCount = rawContent.split(/\s+/).length;

    // Binary formats (docx, pdf) stored as base64 so they survive the text column
    const contentToStore = Buffer.isBuffer(formattedContent)
    ...
    await pool.query(
      `UPDATE manual_reports
       ...
       WHERE id = $4`,
      [contentToStore, wordCount, chunks.length, reportId]
    );
    ...
    return {
      ...
      reportType,
      status: "Ready",
      // Return the human-readable AI text for preview — binary is served via /download
